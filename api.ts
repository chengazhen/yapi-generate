...
class YAPI {
  ...
  parseContent(content: any) {
    ...
    for (let i of content) {
      ...
      str += this._end()
      // 写文件
      this._writeFile(__dirname + '/api/' + i.desc.toLowerCase().replace(/\s+/g, '-') + '.js', str, __dirname + '/api/')
    }
    ...
    data.forEach((item, index) => {
      item.list.forEach(sub_item => {
        ...
        if (regx.test(sub_item.title)) {
          ...
          const list: Array<any> = item.list
          let methods = ''
