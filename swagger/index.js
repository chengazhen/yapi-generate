 ...
 * @LastEditTime: 2021-09-30 10:44:50
...
function checkArgv(argv) {
	...
	argv.forEach((item) => {
		...
		let value = item.match(/(?<==)(?:http(?:s{0,1}):\/\/\d{1,3}(?:\.\d{1,3}){3}:\d{4}(?:\/{0,1})(?:,{0,})){1,}/g)
		if (value) {
			value = value[0]
		} else {
...
const calculateObj = {
	...
	calcUrl: function (paths) {
		...
		temPathsKeys.forEach((item) => {
			...
			if (secKeys.length > 1) {
				...
				urlsObj[prefix[0]] ? urlsObj[prefix[0]].concat(secKeys) : (urlsObj[prefix[0]] = secKeys)
			} else {
				urlsObj[prefix[0]] ? urlsObj[prefix[0]].push(item) : (urlsObj[prefix[0]] = [item])
			}
		})
		...
		for (const key in values) {
			...
			if (Object.keys(element).length) {
				for (const eKey in element) {
					...
					if (obj.consumes) {
						info.headers = {
							'Content-Type': obj.consumes.length === 1 ? obj.consumes + '' : obj.consumes,
						}
					}
	...
	calcParameters: function (params, method) {
		...
		params.forEach((item) => {
			...
			let definitions = (item?.schema?.$ref ?? '') || (item?.schema?.items?.$ref ?? '')
			const obj = {
...
`
			...
			for (const k in element) {
				...
				// 剩余信息rest
				const { paramStr, paramTypeStr, funsParamStr } = calculateObj.checkParam(param)
